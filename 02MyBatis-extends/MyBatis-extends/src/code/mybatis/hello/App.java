...
public class App {
	...
	void testGetCount() throws Exception {
		SqlSession session = MyBatisUtil.getSession();
		int rows = session.selectOne("code.mybatis.hello.UserMapper.queryForCount");
		session.close();
	...
	void testGetMap() throws Exception {
		SqlSession session = MyBatisUtil.getSession();
		List<Map<String,Object>> us = session.selectList("code.mybatis.hello.UserMapper.queryForMap");
		session.close();
